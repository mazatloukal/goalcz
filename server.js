...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    } else if (pathname === '/competition-matches') {
      // Zápasy soutěže – používá se pro pavouk UCL/UEL/UECL
      const id = url.searchParams.get('id');
      if (!id) { res.statusCode = 400; res.end(JSON.stringify({ error: 'Missing id' })); return; }
      const params = [];
      ['status','stage','matchday','dateFrom','dateTo'].forEach(p => {
        const v = url.searchParams.get(p);
        if (v) params.push(`${p}=${encodeURIComponent(v)}`);
      });
      const qs = params.length ? `?${params.join('&')}` : '';
      const cacheKey = `comp_matches_${id}${qs}`;
      let data = getCache(cacheKey);
      if (!data) {
        data = await apiRequest(`/competitions/${id}/matches${qs}`);
        setCache(cacheKey, data, 5 * 60 * 1000); // 5 minut
      }
      res.end(JSON.stringify(data));
